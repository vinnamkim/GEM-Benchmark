...
if __name__ == '__main__':
    ...
    parser.add_argument('-lexp', '--load_exp', default=False,
                        help='load saved experiment results (default: False)')
    parser.add_argument('-node_labels', '--node_labels', default=False,
                        help='node labels available or not (default: False)')
    parser.add_argument('-rounds', '--rounds', default=5,
                        help='number of rounds (default: 5)')
    parser.add_argument('-plot', '--plot', default=True,
                        help='plot the results (default: True)')
    parser.add_argument('-plot_d', '--plot_d', default=True,
                        help='plot the results wrt dims(default: True)')
    parser.add_argument('-hyp_plot', '--hyp_plot', default=True,
                        help='plot the hyperparameter results (default: True)')
    parser.add_argument('-hyp_plot_all', '--hyp_plot_all', default=True,
                        help='plot the hyperparameter results (all) (default: True)')
    parser.add_argument('-train_ratio_lp', '--train_ratio_lp', default=0.8,
                        help='fraction of data used for training(default: 0.8)')
    parser.add_argument('-viz_params', '--viz_params', default='gr',
                        help='which params to use for viz (default: gr)')
    parser.add_argument('-find_hyp', '--find_hyp', default=False,
                        help='find best hyperparameters (default: False)')
    parser.add_argument('-saveMAP', '--save_MAP', default=False,
                        help='save MAP in a latex table (default: False)')
    parser.add_argument('-n_samples', '--n_sample_nodes', default='1024',
                        help='number of sampled nodes (default: 1024)')
    parser.add_argument('-s_sch', '--samp_scheme', default='u_rand',
